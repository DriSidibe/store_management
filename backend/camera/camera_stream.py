...
def generate_thumbnail(video_path, output_dir=THUMBNAILS_DIR, thumbnail_count=1, quality=85):
    ...
    for i, pos in enumerate(frame_positions):
        ...
        if ret:
            ...
            filename = f"thumbnail_{os.path.basename(video_path).split('.')[0]}.jpg"
...
class CameraStream:
    ...
    def start_recording(self, output_dir=RECORDINGS_DIR):
        if self.recording:
            return False
...
def reinitialize_camera_streams():
    ...
    try:
        with open(CAMERAS_CONFIG_PATH, 'r') as file:
